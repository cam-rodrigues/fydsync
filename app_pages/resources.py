...
A branded header matching your other FidSync pages
Search by resource name, category, or domain
A category filter
Total-resource and category counts
Resource names and domains beneath each logo
A fallback letter when an external logo fails to load
External-link indicators
Smoother card hover effects
Responsive four-, two-, and one-column layouts
A proper empty state when nothing matches
A cleaner bottom request callout
No second st.set_page_config() call inside the imported page
it hasnt updated at all in the streamlit it looks exactly the same

That usually means Streamlit is still loading the old resources.py, not that the new styling failed.

Check these in order:

Make sure you replaced the file at exactly:
app_pages/resources.py

Your app.py navigation loads:
...
and your page loader looks inside:

PAGES_DIR = "app_pages"

So a file saved as either of these will not be used:

pages/resources.py
resources.py
app_pages/Resources.py

The capitalization and folder both matter.

Confirm the saved file contains this near the top:
# app_pages/resources.py

from html import escape
from urllib.parse import urlparse

The upgraded version should not begin with:
...
It still uses components, but it should also have the new imports and the large CATEGORIES constant outside run().

Restart Streamlit completely. Stop the terminal with:
Ctrl+C
...
A browser refresh is sometimes not enough after replacing imported page modules.

If you are using GitHub and Streamlit Community Cloud, make sure you actually committed the updated file:
Add file / edit file
Commit changes

Then open your Streamlit deployment and choose:

Manage app → Reboot app

Also confirm the latest commit visible in GitHub includes the new app_pages/resources.py.

Add a temporary test line at the beginning of run():
def run():
    st.success("NEW RESOURCES PAGE IS LOADING")
    apply_page_styles()

If that message does not appear, Streamlit is definitely loading a different file.

Check whether you have two resources files in the repository, such as:
app_pages/resources.py
pages/resources.py
Resources.py
resources.py

The router only uses app_pages/resources.py.

A quick definitive test is to temporarily change the heading inside the upgraded file:

<div class="resources-header-title">
    TESTING NEW RESOURCES PAGE
</div>

If that does not appear after a full Streamlit restart, the wrong file or deployment version is being loaded.

Pasted text(206).txt
Document
ok so its working now but theres a bunch of raw text 

The raw text is happening because several multiline HTML blocks are indented inside Python. Markdown interprets those leading spaces as code blocks before rendering the HTML.

I fixed the page by applying textwrap.dedent() to all of the HTML passed into st.markdown(). That preserves your design but removes the indentation that causes the raw code to appear.

resources_fixed_no_raw_text.py

Replace your existing:

app_pages/resources.py

with the downloaded file, then restart Streamlit.

The key fix added was:

from textwrap import dedent

and HTML blocks now use:
...
instead of passing the indented triple-quoted string directly. 
