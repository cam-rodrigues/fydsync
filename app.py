does show some raw code
should the indents be like this
Pasted text(195).txt
Document
ok what else should change about this - 
when i hit clear search, this happened 
Pasted text(196).txt
Document
ok now lets add some to the article analyser please 
Pasted text(197).txt
Document
can you help me add easter eggs to app.py
when i open it to the home page it looks likke this but if i then go to another tool and click back to the home page it looks normal 
Pasted text(198).txt
Document
ok can you resend the app.py with that fixed, this is what i have now - 
Pasted text(200).txt
Document
this is my app.py - 
Pasted text(201).txt
Document
ok i added them to assets with the names you used can you gimme a new app.py using those, this is what i have in app.py now
i like it alot but it looks kinda blurry or is that just me 

Yes. You can add break reminders that appear as st.toast() messages while the FidSync tab remains open.

Streamlit does not continuously execute Python after a page finishes loading, so the app needs a small automatic-refresh component to trigger periodic reruns. Session State can then remember when each user last received a reminder across those reruns.

1. Add this to requirements.txt
streamlit-autorefresh
2. Add these imports to app.py
from datetime import datetime, timedelta
import random

from streamlit_autorefresh import st_autorefresh
3. Add these settings near the top of app.py
BREAK_REMINDER_MINUTES = 60

BREAK_REMINDER_MESSAGES = [
    "Coffee break reminder: step away for a few minutes.",
    "You have been looking at the screen for a while. Give your eyes a break.",
    "Quick reset: stand up, stretch, and look away from the screen.",
    "Time for a short break. The spreadsheets will still be here.",
    "Hydration check: grab some water and move around for a minute.",
    "Screen break: look at something farther away for a little while.",
    "Pause the analysis. Your eyes and shoulders could use a reset.",
]
4. Add these values inside initialize_app_state()

Change your existing defaults to include:
...
5. Add this function beneath your other helper functions
def check_break_reminder() -> None:
    """Show a break reminder after the configured amount of active time."""

    if not st.session_state.break_reminders_enabled:
        return

    now = datetime.now()
    last_reminder = st.session_state.last_break_reminder

    time_since_last_reminder = now - last_reminder
    reminder_interval = timedelta(
        minutes=BREAK_REMINDER_MINUTES
    )
...
6. Add the automatic refresh

Place this after initialize_app_state():

# Rerun the app once per minute so reminder timing can be checked.
st_autorefresh(
    interval=60 * 1000,
    key="fidsync_break_reminder_refresh",
)

That does not show a reminder every minute. It only gives the app a chance to check the timer every minute.

7. Call the reminder checker
...
8. Add an on/off option in the Version panel

Inside your existing Version 0.9 Beta expander, add:

st.toggle(
    "Break reminders",
    key="break_reminders_enabled",
    help="Show a short wellness reminder after one hour of active app use.",
)

if st.session_state.break_reminders_enabled:
    st.caption(
        f"Next reminder interval: every "
        f"{BREAK_REMINDER_MINUTES} minutes"
    )
...
Full reminder section

The core addition would look like this:
...
def check_break_reminder() -> None:
    ...
    if (
        now - st.session_state.last_break_reminder
        >= reminder_interval
    ):
        st.toast(
            random.choice(BREAK_REMINDER_MESSAGES),
            icon="☕",
        )
...
One limitation: this is an in-app toast, not a Windows or browser push notification. The FidSync tab and its Streamlit session need to remain open. The third-party auto-refresh component also relies on the active browser page to trigger the rerun.

A better rhythm than only one hourly reminder would be a subtle eye-break reminder every 30 minutes and a fuller coffee/stretch reminder every 60 minutes.

Pasted text(202).txt
Document
ok can you resend this whole this, this is what i currently have 

Absolutely. I can do that, but the full app.py is around 450+ lines now. Including the entire updated file with the new break reminder system would exceed the maximum response length that I can send in a single message.

I do have your current version from the file you uploaded , so I can merge the changes correctly.

The easiest way is to send it in 2–3 parts:

Part 1: imports, session state, reminder system, CSS
Part 2: sidebar, navigation, page loader
Part 3: any remaining code (if needed)

That way you'll get the complete updated file with:

☕ Hourly coffee/stretch reminders
👀 Eye strain reminders
💧 Hydration reminders
🔔 Toggle to enable/disable reminders
📊 Counter showing how many reminders have appeared this session
⏱️ Automatic timer using streamlit-autorefresh

and nothing from your current app will be removed.

I don't want to truncate the file or accidentally leave out a section, so splitting it into a couple of messages is the safest approach.

yes please do that

I merged the full break-reminder system into your current app.py without removing the logo setup, navigation, default Getting Started page, developer mode, or system-check Easter egg.

app_with_break_reminders.py

You also need this line in requirements.txt:

streamlit-autorefresh

What was added:

Automatic timing checks once per minute
Break reminders enabled by default
Random coffee, eye-rest, hydration, posture, and stretch messages
Reminder interval choices of 30, 45, 60, or 90 minutes
An on/off toggle inside the Version 0.9 Beta panel
A “Reset Break Timer” button
A count of reminders shown during the current session
Timer resets automatically when the toggle or interval changes

The tab must remain open for the reminders to appear. They are Streamlit toast messages, not Windows push notifications. 
