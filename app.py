...
HOMEPAGE_SUBTITLES = [
    ...
    (
        "Financial research with fewer tabs and slightly fewer headaches."
    ),
    (
        "Organizing market data one suspiciously formatted file at a time."
    ),
    (
        "Built for analysts, advisors, and people who have seen too many "
        "spreadsheets."
    ),
...
def initialize_app_state() -> None:
    ...
    # Initialize this separately so the random choice is only made
    # when the value does not already exist.
    if "app_home_subtitle" not in st.session_state:
        st.session_state.app_home_subtitle = random.choice(
            HOMEPAGE_SUBTITLES
        )
...
def get_time_message() -> str:
    """Return a greeting based on the current date and time."""
...
def show_app_greeting() -> None:
    """Display the homepage greeting once per session."""
...
def register_page_location(selected_page_name) -> None:
    """
    Track actual homepage visits without counting ordinary widget reruns.
    """
...
def show_homepage_achievement() -> None:
    """Unlock an achievement after five homepage visits."""
...
def maybe_show_homepage_thought() -> None:
    """Occasionally show a thought once per actual homepage visit."""
...
st.markdown(
    ...
    <style>
        ...
        .hero-eyebrow {
            ...
            -webkit-text-fill-color: #b9cde5 !important;
            opacity: 1 !important;
        ...
        /*
        This is deliberately a div instead of an h1.

        Streamlit can apply its own heading colors during the initial
        page load. Using a div and forcing the text color prevents the
        title from appearing dark against the navy background.
        */
        .hero-title {
            display: block;
            margin: 0;
